<x-backend>
    ...
    <main class="app-content">
        <div class="app-title">
            <div>
                <h1> <i class="icofont-list"></i> Edit MovieDirection </h1>
            </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="tile">
                    <div class="tile-body">
                        <form action="{{ route('movie_direction.update',$id) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="directorid">
                                        ...
                                        @foreach($directors as $director)
                                            <option value="{{ $director->id }}" @if($director_id == $director->id) {{'selected'}} @endif> {{ $director->name }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
